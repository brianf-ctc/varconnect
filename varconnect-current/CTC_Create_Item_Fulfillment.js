...
//			var PO_Valid_Date = '9/19/2017';
			...
			if (lineData != null && lineData.length > 0) {
				...
				lineData.sort(sort_by('order_num', false, function(a){return a.toUpperCase()}));
...
//				}
				...
				for (var i = 0; i < lineData.length; i++) {
					if (lineData[i].order_num &&
							fulfillmentOrders.indexOf(lineData[i].order_num) < 0)
						...
						if (fulfillmentLines.length > 0){
							try {
								...
								log.error({
									...
									details: 'Could not transform SO ID '+so_ID+' error = '+err.message
								});
							}
							...
							if (objRecord != null){
								...
								for (var cnt = 0; cnt < lineItemCount; cnt++){
									...
									if (!itemInLineData(tempItemNum, fulfillmentLines, tempVendorSKU, mainConfig.ingramHashSpace, vendorConfig.xmlVendor, tempItemLine)){
										...
										log.debug({
											...
											details: 'tempItemNum = ' + tempItemNum + ' line num = ' + cnt
										});
								...
								for (var itemCnt = 0; itemCnt < fulfillmentLines.length; itemCnt++){
									...
									var el = {item_num:"", totalShipped:"0", order_num:"", "order_date":"", "order_eta": "", "ship_date":"", "all_tracking_nums": "", "carrier": "", "all_serial_nums": ""};
...
//									}
									...
									if (!itemInLineData(fulfillmentLines[itemCnt].item_num, uniqueItems, "", mainConfig.ingramHashSpace, vendorConfig.xmlVendor, tempItemLine)){
										...
										for (var uniqueIndex = 0; uniqueIndex < uniqueItems.length; uniqueIndex++){
											if (fulfillmentLines[itemCnt].item_num == uniqueItems[uniqueIndex].item_num) {
												uniqueItems[uniqueIndex].totalShipped += parseInt(fulfillmentLines[itemCnt].ship_qty);
												...
												if (typeof tempTrackingNums == 'string')
													tempTrackingNums = fulfillmentLines[itemCnt].tracking_num.split(',');
												for (var tnIndex = 0; tnIndex < tempTrackingNums.length; tnIndex++){
													if (uniqueItems[uniqueIndex].all_tracking_nums.indexOf(tempTrackingNums[tnIndex]) < 0){
												...
												if (typeof tempSerials == 'string')
													tempSerials = fulfillmentLines[itemCnt].serial_num.split(',');
												for (var snIndex = 0; snIndex < tempSerials.length; snIndex++){
													if (uniqueItems[uniqueIndex].all_serial_nums.indexOf(tempSerials[snIndex]) < 0){
								...
								for (var cnt2 = 0; cnt2 < lineItemCount2; cnt2++){
									...
									if (received2){
										var item = {item_num:"", totalShipped:"0", order_num:"", "order_date":"", "order_eta": "", "ship_date":"", "all_tracking_nums": "", "carrier": "", "all_serial_nums": ""};
										var currItemNum = objRecord.getCurrentSublistValue({
											sublistId: 'item',
											fieldId: vcGlobals.ITEM_FUL_ID_LOOKUP_COL
										});
										var currItemQty = parseInt(objRecord.getCurrentSublistValue({
											sublistId: 'item',
											fieldId: 'quantity'
										}));
										...
										for (var tmp2 = 0; tmp2 < uniqueItems.length; tmp2++){
											...
											if (!uniqueItems[tmp2].line_num ||
													uniqueItems[tmp2].line_num == 'NA' ||
													uniqueItems[tmp2].line_num == tempItemLine) {

												if (currItemNum == uniqueItems[tmp2].item_num ||  (currVendorSKU != "" && currVendorSKU == uniqueItems[tmp2].vendorSKU)) {
													if (currItemQty < parseInt(uniqueItems[tmp2].totalShipped)){
														uniqueItems[tmp2].totalShipped -= parseInt(currItemQty);
	
														updateXMLField(objRecord, uniqueItems[tmp2].order_num, 'custcol_ctc_xml_dist_order_num')
														updateXMLField(objRecord, uniqueItems[tmp2].order_date, 'custcol_ctc_xml_date_order_placed')
														updateXMLField(objRecord, uniqueItems[tmp2].order_eta, 'custcol_ctc_xml_eta')
														updateXMLField(objRecord, uniqueItems[tmp2].ship_date, 'custcol_ctc_xml_ship_date')
														updateXMLField(objRecord, uniqueItems[tmp2].carrier, 'custcol_ctc_xml_carrier')
														updateXMLField(objRecord, parseDate({ dateString: uniqueItems[tmp2].order_eta }), 'custcol_ctc_vc_eta_date')
														updateXMLField(objRecord, parseDate({ dateString: uniqueItems[tmp2].order_date }), 'custcol_ctc_vc_order_placed_date')
														updateXMLField(objRecord, parseDate({ dateString: uniqueItems[tmp2].ship_date }), 'custcol_ctc_vc_shipped_date')
	...
	//															"all_tracking_nums": "", 
								...
								try {
									if (rec_Changed){
										...
										responseData.push({id: objId, orderNum: fulfillmentOrders[j]})
									}
									else {
									...
									log.error({
										title: 'Create Item Fulfillment',
										details: 'Could not save item fulfillment error = '+err.message
									});
								}
...
//					log.debug(tempItemNum + ' = ' + lineData[i].item_num, tempVendorSKU + ' = ' + lineData[i].vendorSKU);
					...
					if (lineData[i] && (
						...
						log.debug(tempItemNum == lineData[i].item_num, tempItemNum + ' = '+ lineData[i].item_num);
