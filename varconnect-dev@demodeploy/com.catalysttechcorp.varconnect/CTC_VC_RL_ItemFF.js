    ...
    var RESTLET = {
        post: function (scriptContext) {
            ...
            try {
                ...
                log.debug(
                    logTitle,
                    vc_util.getUsage() + LogPrefix + '// PO Data: ' + JSON.stringify(PO_Data)
                );
                ...
                for (i = 0; i < arrOrderNums.length; i++) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            log.debug(
                                logTitle,
                                vc_util.getUsage() + LogPrefix + '// transform success.'
                            );
                        } catch (transform_err) {
                            Helper.logMsg({
                        ...
                        for (line = 0; line < lineItemCount; line++) {
                            ...
                            if (lineFFData.item_po != Current.PO_ID) {
                                log.debug(
                                    logTitle,
                                    vc_util.getUsage() + LogPrefix + '.... removed: not same PO '
                                );
                        ...
                        for (line = 0; line < lineItemFFCount; line++) {
                            ...
                            if (!lineFFData.isReceived) {
                                log.debug(
                                    logTitle,
                                    vc_util.getUsage() + (LogPrefix + '....skipping: not received.')
                                );
                            ...
                            for (ii = 0; ii < UniqueLineItems.length; ii++) {
                                ...
                                if (
                                    ...
                                    log.debug(
                                        logTitle,
                                        vc_util.getUsage() + (LogPrefix + '....selected')
                                    );
                                    ...
                                    log.audit(
                                        logTitle,
                                        vc_util.getUsage() + LogPrefix + '...skipped.'
                                    );
                            }
    ...
    var ItemFFLib = {
        ...
        validateOrderList: function (option) {
            ...
            try {
                ...
                log.audit(
                    logTitle,
                    vc_util.getUsage() + LogPrefix + '.. sorted: ' + JSON.stringify(sortedOrders)
                );
