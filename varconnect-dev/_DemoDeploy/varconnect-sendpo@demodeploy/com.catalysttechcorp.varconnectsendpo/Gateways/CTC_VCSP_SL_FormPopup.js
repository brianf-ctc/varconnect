        ...
        if (!CTC_Util.isEmpty(returnValue)) {
            ...
            switch (type) {
                ...
                case 'CURRENCY':
                    if (
                        option.value &&
                        option.value.indexOf('_vcsp_item.') == 0 &&
                        option.transaction &&
                        option.line >= 0
                    ) {
                        switch (option.value.slice(11)) {
                            case 'rate':
                                returnValue = option.transaction.getSublistValue({
                                    sublistId: 'item',
                                    fieldId: 'rate',
                                    line: option.line
                                });
                ...
                if (poField.fieldGroup) {
                    ...
                    if (poField.sublist) {
                        if (!sublist) {
                            ...
                            if (record) {
                                // columnDisplayType = NS_ServerWidget.FieldDisplayType.INLINE;
                                columnDisplayType = NS_ServerWidget.FieldDisplayType.DISABLED;
                            }
                    ...
                    if (poField.defaultValue) {
                        if (poField.sublist) {
                            for (
                                let i = 0, len = lineCount || record.getLineCount('item');
                                i < len;
                                i += 1
                            ) {
                                ...
                                fieldDetails.defaultValue = parsedValue;
                                log.debug(
                                    logTitle,
                                    [
                                        'Adding to ' + poField.sublist,
                                        poField.name,
                                        'field ~',
                                        JSON.stringify(fieldDetails)
                                    ].join(' ')
            ...
            if (vendorConfig) {
                if (context.request.method == 'POST' && poid) {
                    ...
                    if (fieldsArrayStr) {
                        ...
                        CTC_Util.log('DEBUG', logTitle, poObj);
                        let fieldsArr = CTC_Util.safeParse(
                            CTC_SSUtil.renderTemplate({
                                body: fieldsArrayStr,
                                purchaseOrder: poObj
                            })
                        );
