...
define(function (require) {
    ...
    function getInvoiceDetails(recordId, config) {
        ...
        var invTranId = getInvoiceTranId(recordId);
        var objHeaders = {
            Authorization: getTokenCache(config),
            'Content-Type': 'application/json'
        };
        var invoiceListUrl =
            config.url + '/Invoice/DocNumber=' + invTranId + ',Order_ID=' + config.poNum;
    ...
    function getTokenCache(config) {
        ...
        var cacheKey = 'CARASOFT_TOKEN-' + vc2_util.convertToQuery(extraParams),
    ...
    function getInvoiceTranId(recordId) {
        if (vc2_util.isEmpty(recordId)) {
