						...
						if (receiptLines.length > 0){
							try {
								...
								log.error({
									...
									details: 'Could not transform po ID '+po_ID+' error = '+err.message
								});
							}
							...
							if (objRecord != null){
								...
								for (var itemCnt = 0; itemCnt < receiptLines.length; itemCnt++){
									...
									if (!itemInLineData(receiptLines[itemCnt].item_num, uniqueItems, "")){
								...
								for (var tmp3 = 0; tmp3 < uniqueItems.length; tmp3++){
									var found= false;
									var trackingField = mainConfig.useInboundTrackingNumbers ? constants.Columns.INBOUND_TRACKING_NUM : 
