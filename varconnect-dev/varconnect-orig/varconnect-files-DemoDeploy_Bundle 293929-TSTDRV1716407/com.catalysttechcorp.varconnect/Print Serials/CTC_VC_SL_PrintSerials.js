...
function(render,
	...
	function _searchSerials(recType, recId) {
		var name = (recType == record.Type.INVOICE) ? 'custrecordserialinvoice' : 'custrecorditemfulfillment',
			filters = [{
				name: name,
				operator: 'anyof',
				values: recId
	...
	function _addSalesOrder(options) {
		...
		if (recType == record.Type.ITEM_FULFILLMENT) {
			...
			var salesOrder;
			if (lookup &&
					lookup.createdfrom[0] &&
					lookup.createdfrom[0].value)
    	...
    	if (context.request.method === 'GET') {
			...
			if (mainConfig.invPrintSerials) {
	    		var rec = record.load({
					...
					scriptId = (recType == record.Type.INVOICE) ? mainConfig.printSerialsTemplate : TEMPLATES.PACKING,
					sublistId = (recType == record.Type.INVOICE) ? 'recmachcustrecordserialinvoice' : 'recmachcustrecorditemfulfillment';
