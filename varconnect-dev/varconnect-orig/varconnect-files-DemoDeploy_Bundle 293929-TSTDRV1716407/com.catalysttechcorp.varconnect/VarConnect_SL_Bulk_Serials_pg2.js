            ...
            if (!isEmpty(poField)){
                ...
                if (!isEmpty(itemsField)) {
                    ...
                    for (var i = 0; i < itemsField.length; i++){
                        ...
                        log.debug ({title: 'In POST code', details: 'Item ' + itemsField[i].id + ' New SNs = '+newSNStr});
                        if (!isEmpty(newSNStr)){
                ...
                if (!isEmpty(mrTask)){
                    ...
                    log.debug ({title: 'In POST code', details: 'Could not Create/Submit the Map/Reduce Task '});
                    var form = 	ui.createForm({
...
//            fileObj.folder = PO_SERIAL_NUMBER_FOLDER;
        ...
        function setSNLineLink(poID){
            ...
            for (var i = 0; i < lineCount; i++) {
                var itemName = encodeURIComponent(poRec.getSublistText({
                    sublistId: 'item',
                    fieldId: 'item',
                    line: i
                }))
