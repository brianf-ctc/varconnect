...
function _validateLicenseParams(options) {
	...
	if (licenseProduct != PRODUCT_CODE) {
		nlapiLogExecution('ERROR', 
				'Invalid License Key - TERMINATING SCRIPT', 'Reason: Mismatch Product ID. Are you sure this license is for the correct catalyst product? License Product ID: ' + licenseProduct + '  Current Product ID: ' + PRODUCT_CODE);
		result = 'invalid';
	} else if (licenseAccountId && accountId && licenseAccountId.toLowerCase() != accountId.toLowerCase()) {
		nlapiLogExecution('ERROR', 
				'Invalid License Key - TERMINATING SCRIPT', 'Reason: Mismatch Netsuite Account. Are you sure this license is for this customer? License Account ID: ' + licenseAccountId + '  Current Account ID: ' + accountId);
		result = 'invalid';
	} else {
		var dateDifference = date_difference(dateToday, licenseExpiryDate);
        if (dateDifference < 0) { 
            nlapiLogExecution('ERROR', 
            		'Invalid License Key - TERMINATING SCRIPT', 'Reason: License has expired, please renew. Expiration Date: ' + licenseExpiryDate + ' Current Date: ' + dateToday); 
            result = 'invalid';
        }
	}
