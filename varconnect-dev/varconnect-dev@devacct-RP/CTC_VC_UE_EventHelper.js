    ...
    EventRouter.Action[ns_record.Type.PURCHASE_ORDER] = {
        onBeforeLoad: function (scriptContext, Current) {
            ...
            try {
                ...
                var vendorCfg = vcs_configLib.vendorConfig({ poId: Current.recordId });
                vc2_util.log(logTitle, '// vendorCfg:  ' + JSON.stringify(vendorCfg));

                var billVendorCfg = vcs_configLib.billVendorConfig({ poId: Current.recordId });
                vc2_util.log(logTitle, '// billVendorCfg:  ' + JSON.stringify(billVendorCfg));
