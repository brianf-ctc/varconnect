...
define([
    ...
    function _getItemAvailability(option) {
        ...
        if (itemAvailReq.isError) {
            if (itemAvailReq.errorMsg == 'Received invalid response code - 207') {
                // check what errored out
                var handleItemAvailabilityError = function (option) {
                    var itemAvailReq = option.itemAvailReq,
                        orderLines = option.orderLines;
                    var itemAvailRespBody = VC2_Utils.safeParse(itemAvailReq.RESPONSE);
                    if (!itemAvailRespBody)
                        throw 'Received invalid response code - 207 | Unable to retrieve the item availability';
                    var isItemMatched = false,
                        isSomeAvailabilityFound = false;
                    for (i = 0, j = orderLines.length; i < j; i++) {
                        ...
                        for (ii = 0, jj = itemAvailRespBody.length; ii < jj; ii++) {
                            ...
                            if (
                                ...
                                isItemMatched = true;
                                if (
                                    itemAvailLine.availability &&
                                    itemAvailLine.availability.availabilityByWarehouse
                                ) {
                                    isSomeAvailabilityFound = true;
                                    break;
                                }
                            }
                        }
                    ...
                    if (!isItemMatched) {
                        throw {
                            error: 'Received invalid response code - 207',
                            details:
                                'A response was received that does not contain the matching Ingram Part Number, UPC, and Vendor Part Number'
                        };
                    }
                    if (!isSomeAvailabilityFound) {
                        throw {
                            error: 'Received invalid response code - 207',
                            details:
                                'A response was received that does not contain the availability by warehouse'
                        };
                    }
                };
                handleItemAvailabilityError({
                    itemAvailReq: itemAvailReq,
                    orderLines: orderLines
                });
            } else {
                throw itemAvailReq.errorMsg;
            }
        }
        var itemAvailRespBody = VC2_Utils.safeParse(itemAvailReq.RESPONSE);
