...
define([
...
], function (record, search, runtime, email, constants, libMainConfig, libLicenseValidator) {
    ...
    function _sendEmail(options) {
        ...
        if (recType == record.Type.ITEM_RECEIPT) cc = cc.concat(CC_ITEM_RECEIPT);
        else if (recType == record.Type.ITEM_FULFILLMENT) cc = cc.concat(CC_ITEM_FULFILLMENT);
    ...
    function getInputData() {
        ...
        if (recordType == 'itemreceipt') {
            ...
            if (lookup) {
                if (lookup.type && lookup.type.length > 0 && lookup.type[0].value == 'RtnAuth') {
    ...
    function reduce(context) {
        ...
        if (!!serial)
            if (command == 'create')
                serialId = _createSerial(serial, poId, itemId, soId, ifId, raId, vaId, irId);
