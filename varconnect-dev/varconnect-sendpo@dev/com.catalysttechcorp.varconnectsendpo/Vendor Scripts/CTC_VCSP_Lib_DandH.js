    ...
    function generateBody(option) {
        ...
        dnhTemplate = {
            ...
            endUserData: {
                ...
                // dateOfSale: '',
                address: { // All fields in address are often required if customer has no default address setup with D&H
            ...
            deliveryAddress: {
                address: { // All fields in address are required
        ...
        if (responseBody) {
            ...
            if (responseBody) {
                ...
                if (responseBody.shipments && responseBody.shipments.length) {
                    for (let orderCtr = 0, orderCount = responseBody.shipments.length; orderCtr < orderCount; orderCtr += 1) {
                        ...
                        if (shipmentDetails.lines && shipmentDetails.lines.length) {
                            ...
                            for (let line = 0, lineCount = shipmentDetails.lines.length; line < lineCount; line += 1) {
                                ...
                                if (lineDetails.clientReferenceData) {
                                    itemDetails.internal_reference_num = JSON.stringify(lineDetails.clientReferenceData);
                                }
            ...
            if (option.returnResponse.responseCode >= 300) {
                ...
                if (orderStatus.successLines.length && orderStatus.errorLines.length) {
                    returnValue.message = 'Send PO partially successful with ' +
