...
define((require) => {
	...
	const processResponse=(option)=>{
		...
		if (responseBody) {
			...
			if (returnValue.isError || !returnValue.responseCode || returnValue.responseCode < 200 || returnValue.responseCode >= 300) {
					...
					for (let errCtr = 0, errCount = responseBody.Errors.length; errCtr < errCount; errCtr += 1) {
						...
						if (errorResponse) {
							...
							if (errorResponse.ErrorMessage) {
								errorMessage = errorCode ? '(' + errorCode + ') ' + errorResponse.ErrorMessage : errorResponse.ErrorMessage;
							}
						}
						if (errorMessage) {
	...
	const getSalesRep=(record)=>{
		...
		if(!CTC_Util.isEmpty(soId)){
			...
			if(!CTC_Util.isEmpty(objLookUpSo)){
				...
				if(Array.isArray(arrSalesRep) && typeof arrSalesRep[0]!=="undefined" && typeof arrSalesRep[0] === 'object'){
