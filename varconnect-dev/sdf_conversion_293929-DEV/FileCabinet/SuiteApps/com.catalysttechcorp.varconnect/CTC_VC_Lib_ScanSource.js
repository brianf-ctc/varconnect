 ...
 */
define([
    'N/log', 'N/https', './CTC_VC_Lib_Log.js'
  ],
  function(
    log, https, vcLog
  ) {
    ...
    function processRequest(obj) {
      ...
      var listResponse = https.get({
        url: obj.vendorConfig.endPoint + '/list?customerNumber=' + obj.vendorConfig.customerNo + '&poNumber=' + obj.poNum,
        headers: headers
      });
      ...
      if (listResponse.code == 200) {
        ...
        listResponseBody.forEach(function(order) {
          ...
          var detailResponse = https.get({
            url: obj.vendorConfig.endPoint + '/detail?salesOrderNumber=' + order.OrderNumber + '&customerNumber=' + obj.vendorConfig.customerNo + '&excludeSerialTracking=false',
            headers: headers
          });
      ...
      obj.responseBody.forEach(function(order) {
        ...
        order.Deliveries.forEach(function(delivery) {
          ...
          delivery.LineItems.forEach(function(line) {
            ...
            o.line_num = Math.floor(delivery.DeliveryDocumentNumber + '.' + line.DeliveryDocumentLineNumber) * 1;
