        ...
        post: function (context) {
            ...
            try {
                ...
                CURRENT.billPayload.lines.forEach(function (billLine) {
                    ...
                    if (billLine.AVAILQTY > 0)
                        arrUnprocessedLines.push(billLine.ITEMNO);
                    return true;
                });
                ...
                for (var line = lineCount - 1; line >= 0; line--) {
                    ...
                    lineData.matchingLines.forEach(function (billLine) {
                        ...
                        if (billLine.SERIAL && billLine.SERIAL.length)
                            arrLineSerials = arrLineSerials.concat(
                                billLine.SERIAL
                            );
                        if (billLine.TRACKING && billLine.TRACKING.length)
                            arrLineTracking = arrLineTracking.concat(
                                billLine.TRACKING
                            );
                    ...
                    if (
                        ...
                        (lineData.inventorydetailreq == 'T' ||
                            lineData.isserial == 'T')
                    ) {
                        arrLineSerials.forEach(function (serial) {
                            if (totalAppliedQty <= 0) return;
                            if (vc2_util.inArray(serial, arrAppliedSerials))
                                return;
                    ...
                    if (!objItemSerials[lineData.item])
                        objItemSerials[lineData.item] = [];
