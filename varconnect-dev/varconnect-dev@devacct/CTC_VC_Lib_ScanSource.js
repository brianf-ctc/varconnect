...
define(function (require) {
    ...
    var vcLog = require('./CTC_VC_Lib_Log.js');
    var EntryPoint = {};

    EntryPoint.process = function (options) {
        log.audit('options', options);

        var outputArray = null;

        var responseBody = EntryPoint.processRequest({
            poNum: options.poNum,
        ...
        log.emergency('outputArray', outputArray);
        return outputArray;
    };

    EntryPoint.processRequest = function (obj) {
        //get token
        ...
        //get po response
        var listResponse = getPoResponse(obj, headers);

        var returnArray = [];
        if (listResponse.code == 200) {
            var listResponseBody = JSON.parse(listResponse.body);
            log.debug('List Response Body', listResponseBody);

            listResponseBody.forEach(function (order) {
                //get detail or transaction list response
                var detailResponse = getDetailResponse({
                    order: order,
                    obj: obj,
                    headers: headers
                });
                ...
                if (detailResponse) {
                    returnArray.push(detailResponse);
                }
            });
        } else {
            log.error({
    ...
    function getPoResponse(obj, headers) {
    ...
    function getDetailResponse(options) {
        ...
        if (detailResponse.code == 200) {
            log.debug('Detail Response Body', detailResponse.body);
