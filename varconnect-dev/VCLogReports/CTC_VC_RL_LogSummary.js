...
define((require) => {
	...
	EntryPoint.get=(requestBody)=>{
		...
		try{
			...
			//remove extra 'and'
			if(arrFilters.length>=2){ arrFilters.pop(); }
			...
			if(logReportsPage.pageRanges.length>0){
		        ...
		        logReportsData.data.forEach((result)=>{
					objReturn.data.push({
						...
						'custrecord_ctc_vclog_application':result.getValue('custrecord_ctc_vclog_application'),
						...
						'custrecord_ctc_vclog_vendorconfig':result.getValue('custrecord_ctc_vclog_vendorconfig'),
						'custrecord_ctc_vclog_transaction':result.getValue('custrecord_ctc_vclog_transaction'),
						'custrecord_ctc_vclog_transaction_text':result.getText('custrecord_ctc_vclog_transaction'),
						'custrecord_ctc_vclog_type':result.getValue('custrecord_ctc_vclog_type'),
						'custrecord_ctc_vclog_category':result.getValue('custrecord_ctc_vclog_category'),
						'custrecord_ctc_vclog_key':result.getValue('custrecord_ctc_vclog_key'),
						'custrecord_ctc_vclog_account':result.getValue('custrecord_ctc_vclog_account'),
						'custrecord_ctc_vclog_compname':result.getValue('custrecord_ctc_vclog_compname')
					});
		        });
			}
