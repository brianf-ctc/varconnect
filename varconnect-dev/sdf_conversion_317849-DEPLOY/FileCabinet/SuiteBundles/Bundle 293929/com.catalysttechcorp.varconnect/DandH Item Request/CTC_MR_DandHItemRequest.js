...
function(search,
    ...
    function getInputData() {
    	...
    	if (_checkDandHVendorConfig()) {
    		var srchId = runtime.getCurrentScript().getParameter('custscript_ctc_vc_dh_itemrequest_srch');
    		if (!srchId)
	    		srchId = 'customsearch_ctc_vc_dh_itemrequest';
    ...
    function map(context) {
    	...
    	if (vendorConfig) {
    		...
    		if (itemNumbers) {
    			...
    			if (itemNumbers.itemNum == itemName)
    				valToSave = itemNumbers.partNum;
    			else if (itemNumbers.partNum == itemName)
	...
	function _processItemInquiryResponse(options) {
        ...
        if (xmlDoc != null){
        	...
        	if (itemNode != null && itemNode.length > 0) {
        		itemNum = util.getNodeTextContent(xml.XPath.select({node:itemNode[0], xpath:'VENDORITEMNO'})[0]);
        		partNum = util.getNodeTextContent(xml.XPath.select({node:itemNode[0], xpath:'PARTNUM'})[0]);
        	}
        }
