...
function(serverWidget, 
	...
	function _validateLicense(options) {
		...
		if (response == 'valid') {
			licenseText = "<span style='background-color:lightgreen'><b>VERIFIED: Your License for VAR Connect is currently valid.</b></span>";
    ...
    function beforeLoad(scriptContext) {
    	...
    	if (scriptContext.type === scriptContext.UserEventType.CREATE || 
    		...
    		var newRecord = scriptContext.newRecord,
    			isProcessDropship = newRecord.getValue({ fieldId: constants.Fields.MainConfig.PROCESS_DROPSHIPS }),
    			isProcessSpecialOrder = newRecord.getValue({ fieldId: constants.Fields.MainConfig.PROCESS_SPECIAL_ORDERS });
   		
    		if (!isProcessDropship)
    			_disableAndClearFields({
    				form: scriptContext.form, 
    				fields: [constants.Fields.MainConfig.CREATE_ITEM_FULFILLMENTS, constants.Fields.MainConfig.IGNORE_DIRECT_SHIPS_DROPSHIPS]
    			});
    		if (!isProcessSpecialOrder)
    			_disableAndClearFields({
    				form: scriptContext.form, 
    				fields: [constants.Fields.MainConfig.CREATE_ITEM_RECEIPTS, constants.Fields.MainConfig.IGNORE_DIRECT_SHIPS_SPECIAL_ORDERS]
    			});
    	}
