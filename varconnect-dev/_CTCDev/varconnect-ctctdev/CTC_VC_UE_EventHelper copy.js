    ...
    EventRouter.Action[ns_record.Type.PURCHASE_ORDER] = {
        onBeforeLoad: function (scriptContext, Current) {
            ...
            try {
                ...
                var currentRecord = scriptContext.newRecord,
                    currentForm = scriptContext.form,
                    currentSublistItem = currentForm.getSublist({ id: 'item' });
                ...
                vc2_util.log(logTitle, 'SET FIELD VALUES: ', [fieldValues]);

                // get the field
                var newFieldObj = currentForm.addField({
                    id: 'custpage_ctc_column_settings',
                    label: 'Column Settings',
                    type: 'textarea'
                });
                newFieldObj.defaultValue = JSON.stringify(fieldValues);

                // var fldValuesObj = currentForm.getField({ id: 'custbody_ctc_vc_helper_field' });
                // fldValuesObj.defaultValue = JSON.stringify(fieldValues);
                // fldValuesObj.value = JSON.stringify(fieldValues);

                // currentRecord.setValue({
                //     fieldId: 'custbody_ctc_vc_helper_field',
                ...
                if (!mainCfg.overridePONum || !vendorCfg) {
                    Helper.hideFields(currentForm, ['custbody_ctc_vc_override_ponum']);
                }
        ...
        onBeforeSubmit: function (scriptContext, Current) {
            ...
            try {
                ...
                var fieldSettings = currentRecord.getValue({
                    fieldId: 'custpage_ctc_column_settings'
                });

                vc2_util.log(logTitle, '*** BEFORE SUBMIT ***', fieldSettings);
                ...
                //     currentForm = scriptContext.form;
                // // check for the form
                // var fieldValues = JSON.stringify(vc_record.FieldMapping.lineColumn);
                // vc2_util.log(logTitle, 'SET FIELD VALUES: ', [fieldValues]);
                // // get the field
                // var fldValuesObj = currentForm.getField({ id: 'custbody_ctc_vc_helper_field' });
                // fldValuesObj.defaultValue = fieldValues;
                // currentRecord.setValue({
                //     fieldId: 'custbody_ctc_vc_helper_field',
                //     value: fieldValues
                // });
