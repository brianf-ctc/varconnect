...
<pdf>
			...
			<macro id="nlheader">
                <table class="header" style="width: 100%;" table-layout="fixed">
                    <tr>
                        ...
                        <td rowspan="2" colspan="1"><span class="nameandaddress">${subsidiary.name}</span><br /><span class="nameandaddress">${subsidiary.mainaddress_text}</span></td>
                        <td width="30%" align="right" colspan="1"><span class="title">${record@title}</span></td>
                    </tr>
                    <tr>
                        <td>
	...
	<body header="nlheader" header-height="10%" footer="nlfooter" footer-height="10pt" padding="0.5in 0.5in 0.5in 0.5in" size="Letter">
        ...
        <table align="right" style="width: 100%; padding-top: 40px;">
            <tr>
                <td border-top="1" border-right="1" border-left="1" border-bottom="1" style="background-color: #e3e3e3;"><b>${record.billaddress@label}</b></td>
                <td></td>
                <td border-top="1" border-right="1" border-left="1" border-bottom="1" style="background-color: #e3e3e3;"><b>${record.shipaddress@label}</b></td>
            </tr>
            <tr>
            ...
            <tr>
                <td border-right="1" border-left="1" border-bottom="1" <#if record.custbody_ctc_contact_name!=''>border-top="1"</#if>>
                    ...
                    <b>${record.custbody_ctc_contact_name}</b>
                      <#if record.custbody_ctc_select_contact.email!=''><br />${record.custbody_ctc_select_contact.email} </#if>
                      <#if record.custbody_ctc_select_contact.phone!=''><br />${record.custbody_ctc_select_contact.phone} </#if>
                    </#if>
                </td>
            </tr>
        </table>
		...
		<table style="width: 100%; margin-top: 10px; padding-top: 10px;" table-layout="auto">
			<tr>
				...
				<th border-top="1" border-right="1" border-bottom="1">Shipping<br />Method</th>
			</tr>
			<tr>
				...
				<#list record.item as item>
					...
					<#if (item.custcol3?has_content || item.custcol_ctc_renewalend?has_content)>
						<tr>
							...
							<td colspan="4"><b>RENEWAL START: </b> ${item.custcol3}</td>
							<td border-right="1" colspan="4"><b>RENEWAL END: </b> ${item.custcol_ctc_renewalend}</td>
					...
					<#if (item.custcol_ctc_renewal_serial_numbers?has_content)>
						<tr>
							<td border-right="1" border-left="1" colspan="4">&nbsp;</td>
							<td border-right="1" colspan="1">&nbsp;</td>
							<td border-right="1" colspan="8"><b>RENEWAL SERIALS: </b> ${item.custcol_ctc_renewal_serial_numbers}</td>
							<td border-right="1" colspan="3">&nbsp;</td>
							<td border-right="1" colspan="1">&nbsp;</td>
							<td border-right="1" colspan="3">&nbsp;</td>
						</tr>
		...
		<table border-bottom="1" border-top="1" style="width: 100%;margin-top:5px;">
			<tr>
				...
				<td border-right="1" align="right"><b>${record.subtotal@label}&nbsp;</b></td>
			...
			<tr>
				<td border-right="1" align="right"><b>Shipping &amp; Handling&nbsp;</b></td>
			...
			<#if record.taxtotal?has_content>
				<#assign rowspanCount=rowspanCount + 1 />
			<tr>
				<td border-right="1" align="right"><b>${record.taxtotal@label}&nbsp;</b></td>
			...
			<#if record.total?has_content>
				<#assign rowspanCount=rowspanCount + 1 />
			<tr>
				<td border-right="1" align="right"><b>${record.total@label}&nbsp;</b></td>
			...
			<#if record.amountremaining?has_content>
				<#assign rowspanCount=rowspanCount + 1 />
			<tr>
				<td border-right="1" align="right"><b>${record.amountremaining@label}&nbsp;</b></td>
				...
				<#list VC_InvoiceSerials?sort_by("itemName") as serialline>
					...
					<#if (itemName != serialline.itemName) >
						...
						<tr style="margin-top:10px; border-bottom: 1px solid #000;">
							<th colspan="${numCols}">${serialline.itemName} Serial Number(s) </th></tr>
						<tr>
