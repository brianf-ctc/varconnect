...
//					['itemnumber.inventorynumber', 'is', id],
		...
		txnSearch.run().each(function(result) {
			...
			if (soFound && poFound && ifFound && irFound)
				return false;
			else return true;
		});
	...
	function _createCustomSerial(options) {
		...
		log.debug('createcustom', JSON.stringify({
			id: id,
			item: item,
			serial: serial,
			so: so,
	...
	function _updateCustomSerial(options) {
		...
		if (customSerial &&
	    	...
	    	if (fulfil && !rec.getValue({ fieldId: 'custrecorditemfulfillment' }))	//3.00
	    		rec.setValue({
		    		fieldId: 'custrecorditemfulfillment',
		    		value:  fulfil
		    	});
	    	if (receipt && !rec.getValue({ fieldId: 'custrecorditemreceipt' }))	//3.00
	    		rec.setValue({
		    		fieldId: 'custrecorditemreceipt',
		    		value:  fulfil
		    	});
	...
	function _processCustomSerial(options) {
		...
		try {
			...
			} else //2.00 Logic for updating the so and po fields of the existing custom serial
				_updateCustomSerial({
    ...
    function getInputData() {
		...
		var srchId = runtime.getCurrentScript().getParameter("custscript_ctc_vc_mr_serialization_srch");
		
		var filters = [],
			columns = ['inventorynumber', 'internalid', 'item', 'custitemnumber_ctc_vc_sn'];

		var filterCustomSerial = filters.push(search.createFilter({
			name: 'custitemnumber_ctc_vc_sn',
			operator: 'anyof',
			value: '@NONE@'
		}));
		
		var filterSO = filters.push(search.createFilter({
		...
		var filterPO = filters.push(search.createFilter({
