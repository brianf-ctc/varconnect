...
define((require) => {
    ...
    const addFields = (objParams) => {
        ...
        if (
            objParams.custpage_application !== 'ORDER STATUS' &&
            !Helper.isEmpty(objParams.custpage_application)
        ) {
        ...
        if (
            objParams.custpage_vendorconfig !== '-' &&
            !Helper.isEmpty(objParams.custpage_vendorconfig)
        ) {
    ...
    const showResults = (objParams) => {
        ...
        if (objParams.custpage_date === 'BY DAY' && !Helper.isEmpty(objParams.custpage_date)) {
            ...
            objParams.custpage_date === 'THIS WEEK' &&
            !Helper.isEmpty(objParams.custpage_date)
        ) {
            arrSelectQuery.push(`to_char(custrecord_ctc_vclog_date, 'YYYY/MM/DD') as d`);
            arrGroupQuery.push(`to_char(custrecord_ctc_vclog_date, 'YYYY/MM/DD')`);

            arrWhereQuery.push(
                `custrecord_ctc_vclog_date >= trunc ( sysdate, 'iw' ) AND  custrecord_ctc_vclog_date  < trunc ( sysdate, 'iw' ) + 5`
            );
            orderQuery = `ORDER BY d DESC`;
        } else if (
            objParams.custpage_date === 'LAST WEEK' &&
            !Helper.isEmpty(objParams.custpage_date)
        ) {
            arrSelectQuery.push(`to_char(custrecord_ctc_vclog_date, 'YYYY/MM/DD') as d`);
            arrGroupQuery.push(`to_char(custrecord_ctc_vclog_date, 'YYYY/MM/DD')`);
            ...
            arrWhereQuery.push(
                `custrecord_ctc_vclog_date >= next_day(trunc(sysdate), 'MONDAY') - 14 and custrecord_ctc_vclog_date < next_day(trunc(sysdate), 'MONDAY') - 7`
            );
            orderQuery = `ORDER BY d DESC`;
        } else if (
            objParams.custpage_date === 'THIS MONTH' &&
            !Helper.isEmpty(objParams.custpage_date)
        ) {
            ...
            arrWhereQuery.push(
                `custrecord_ctc_vclog_date BETWEEN trunc (sysdate, 'mm') AND SYSDATE`
            );
            orderQuery = `ORDER BY d DESC`;
        } else if (
            objParams.custpage_date === 'LAST MONTH' &&
            !Helper.isEmpty(objParams.custpage_date)
        ) {
            arrSelectQuery.push(`to_char(custrecord_ctc_vclog_date, 'YYYY/MM') as d`);
            arrGroupQuery.push(`to_char(custrecord_ctc_vclog_date, 'YYYY/MM')`);
            ...
            arrWhereQuery.push(
                `custrecord_ctc_vclog_date between add_months(trunc(sysdate,'mm'),-1) and last_day(add_months(trunc(sysdate,'mm'),-1))`
            );
            orderQuery = `ORDER BY d DESC`;
        } else if (
            objParams.custpage_date === 'BY MONTH' &&
            !Helper.isEmpty(objParams.custpage_date)
        ) {
            arrSelectQuery.push(`to_char(custrecord_ctc_vclog_date, 'YYYY/MM') as d`);
            arrGroupQuery.push(`to_char(custrecord_ctc_vclog_date, 'YYYY/MM')`);
            ...
            objParams.custpage_date === 'BY QUARTER' &&
            !Helper.isEmpty(objParams.custpage_date)
        ) {
        ...
        if (
            objParams.custpage_application != '-' &&
            !Helper.isEmpty(objParams.custpage_application)
        ) {
            arrWhereQuery.push(
                `(UPPER(custrecord_ctc_vclog_application) = UPPER('${objParams.custpage_application}'))`
            );
        }

        if (objParams.custpage_compname !== '-' && !Helper.isEmpty(objParams.custpage_compname)) {
            arrWhereQuery.push(
                `(UPPER(custrecord_ctc_vclog_compname) = UPPER('${objParams.custpage_compname}'))`
            );
        }
        ...
        if (
            ...
            arrWhereQuery.push(
                `(UPPER(custrecord_ctc_vclog_vendorconfig) = UPPER('${objParams.custpage_vendorconfig}'))`
            );
        }
        ...
        for (let i = 0; i < arrResults.length; i++) {
            ...
            let objLogCategory = arrLogCategories.find(
                (objData) => objData.categoryId === logCatId
            );
