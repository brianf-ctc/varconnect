...
*/
        ...
        if (senderId != null && senderId.length > 0) {
            ...
            log.debug({
                title: 'Scheduled Email Script',
                details: 'Unique email list length = ' + emailsList.length
            });
            ...
            for (var j = 0; j < emailsList.length; j++) {
                ...
                for (var x = 0; x < myResults.length; x++) {
                    var sendTo2 = myResults[x].getValue({
                        name: constants.Fields.Transaction.SEND_SHIPPING_UPDATE_TO,
                        join: 'createdFrom'
                    });
    ...
    function getItemFulfillments(searchId) {
        log.debug({
            title: 'Scheduled Email Script',
            details: 'Starting getItemFulfillments with Search ' + searchId
        });
    ...
    function buildItemTable(bodyList) {
        var thStyle =
            'style="border:1px solid black; border-collapse:collapse; font-size:small; padding: 5px 10px 5px 10px;"';
        var tdStyle =
            'style="border:1px solid black; border-collapse:collapse; font-size:small; padding: 5px 10px 5px 10px;"';
