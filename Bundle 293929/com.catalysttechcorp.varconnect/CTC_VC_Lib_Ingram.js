...
function(search, runtime, r, xml, https, vcGlobals, constants, util) {
	function processRequest(options) {
        ...
        var countryCode = 'MD';
        if (country) {
        	if (country == constants.Lists.COUNTRY.CANADA)
        		countryCode = 'FT';
        } else if (vcGlobals.COUNTRY == 'CA')
	        	countryCode = 'FT';
	...
	function processResponse(options) {
        ...
        if (xmlDoc != null){
            ...
            var orderDateTime = util.getNodeTextContent(xml.XPath.select({node:xmlDoc, xpath:'//OrderEntryDate'})[0])
            var orderNum = util.getNodeTextContent(xml.XPath.select({node:xmlDoc, xpath:'//BranchOrderNumber'})[0])

            for (var i = 0; i < itemNodes.length; i++) {
                var xml_items = {line_num:"NA", item_num:"NA", order_num:"NA", order_date:"NA", order_eta:"NA", ship_date:"NA", ship_qty:"NA", tracking_num:"NA", vendorSKU: "NA", carrier:"NA", serial_num:"NA", order_status: "NA"};
                ...
                var itemNum = util.getNodeTextContent(xml.XPath.select({node:itemLineNode, xpath:'ManufacturerPartNumber'})[0])
                if (itemNum != null && itemNum.length > 0) { xml_items.item_num = itemNum; }

                var vendorSKU = util.getNodeTextContent(xml.XPath.select({node:itemLineNode, xpath:'SKU'})[0])
                if (vendorSKU != null && vendorSKU.length > 0) { xml_items.vendorSKU = vendorSKU; }

                var shipQty = util.getNodeTextContent(xml.XPath.select({node:itemLineNode, xpath:'ShipQuantity'})[0])
                if (shipQty != null && shipQty.length > 0) { xml_items.ship_qty = shipQty; }

//                log.debug('xml_items.ship_qty', xml_items.ship_qty);
                var orderStatus = util.getNodeTextContent(xml.XPath.select({node:orderSuffixNode, xpath:'OrderStatus'})[0])
            	if (orderStatus != null && orderStatus.length > 0) { xml_items.order_status = orderStatus; }
                log.debug('xml_items.order_status', xml_items.order_status);

                var carrier = util.getNodeTextContent(xml.XPath.select({node:orderSuffixNode, xpath:'Carrier'})[0]);
                if (carrier != null && carrier.length > 0) { xml_items.carrier = carrier; }

                var shipDate = util.getNodeTextContent(xml.XPath.select({node:orderSuffixNode, xpath:'OrderShipDate'})[0]);
...
//						serialNum = serialNum.substring(6, serialNum.length);
                ...
                var currentSKUNum = util.getNodeTextContent(xml.XPath.select({node:itemLineNode, xpath:'SKU'})[0]);
                ...
                if (['Invoiced', 'Shipped', 'Billed', 'Paid'].indexOf(xml_items.order_status) < 0) {
                	log.audit('skipping line due to status', 'itemNum: ' + xml_items.item_num + ' | sku: ' + xml_items.vendorSKU);
                	continue;
                }
