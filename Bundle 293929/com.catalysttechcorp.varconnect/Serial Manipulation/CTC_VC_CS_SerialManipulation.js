...
function() {
	...
	function _checkHasDuplicate(serialArray) {
		...
		serialArray.forEach(function(serial) {
			var dup = false;
			if (clearedSerials.indexOf(serial) > -1) 
				dup = true;
			
			if (dup && dupSerials.indexOf(serial) < 0)
				dupSerials.push(serial);
			else
				clearedSerials.push(serial);
		});
	...
	function _checkLineForDuplicate(options) {
		...
		if (lineSerials)
			lineSerialArray = _split(lineSerials);
		
		serials.forEach(function(serial) {
			var dup = false;
			if (lineSerialArray.indexOf(serial) > -1) 
				dup = true;
    	...
    	if (fieldId === FLD_SERIAL_NO || fieldId === FLD_SERIAL_NO_UPDATE) {
			...
			if (serialArray.length > 200) {
	    		...
	    		if (dupSerials && dupSerials.length > 0) {
	    			alert('On item line ' + (line+1) + ', the following serials have duplicate entries within the transaction: \n' + 
    						dupSerials.join('\n'));
	    		}
